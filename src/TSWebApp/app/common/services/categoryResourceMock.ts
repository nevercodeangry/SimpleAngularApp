module app.common {
    ...
    function mockRun($httpBackend: ng.IHttpBackendService): void {
        var categories: app.domain.ICategory[] = [];
        var category: app.domain.ICategory;

        category = new app.domain.Category(1, "Leaf Rake","Leaf rake with 48-inch wooden handle.",
            "http://openclipart.org/image/300px/svg_to_png/26215/Anonymous_Leaf_Rake.png");
        categories.push(category);

        category = new app.domain.Category(2, "Garden Cart","15 gallon capacity rolling garden cart",
            "http://openclipart.org/image/300px/svg_to_png/58471/garden_cart.png");
        categories.push(category);

        category = new app.domain.Category(3, "Saw", "15-inch steel blade hand saw",
            "http://openclipart.org/image/300px/svg_to_png/27070/egore911_saw.png");
        categories.push(category);

        category = new app.domain.Category(4, "Hammer","Curved claw steel hammer",
            "http://openclipart.org/image/300px/svg_to_png/73/rejon_Hammer.png");
        categories.push(category);

        category = new app.domain.Category(5, "Video Game Controller","Standard five-button video game controller",
